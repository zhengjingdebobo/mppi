"""T-shape launch with external ref publisher + local-input MPPI.

Starts:
    - ddr_minimal_sim T-shape scenario
...
Usage:
    ...
    ros2 launch exact_mppi_jax sim_tshape_external_ref_launch.py use_rviz:=false
"""
...
logger = get_logger("exact_mppi_jax_sim_tshape_external_ref_launch")


def generate_launch_description() -> LaunchDescription:
    sim_env_config_arg = DeclareLaunchArgument(
        "sim_env_config",
        default_value="scenario_tshape.yaml",
    ...
    global_ref_node = Node(
        ...
        parameters=[
            {"use_sim_time": True},
            {"mppi_config_file": "tshape_planner.yaml"},
        ],
    )
    ...
    cost_viz_node = Node(
        package="exact_mppi_jax",
        executable="cost_breakdown_viz",
        name="cost_breakdown_viz",
        output="screen",
        parameters=[{"use_sim_time": True}],
        condition=IfCondition(LaunchConfiguration("cost_viz")),
    )
