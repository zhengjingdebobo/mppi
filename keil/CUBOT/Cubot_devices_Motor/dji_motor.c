...
// ================== 保护参数配置 ==================
// M3508 最大电流值为 16384 (对应 20A)
// 调试期间建议限制在 10A (约 8000) 或 12A (约 10000)
#define SAFETY_CURRENT_LIMIT  12000.0f
...
// 电流“变化率限制”
float CurrentRamp(float target, float last, float max_delta)
{
...
void DJIMotorControl()
{
    ...
    static float last_current_ref[DJI_MOTOR_CNT] = {0};
    ...
    for (size_t i = 0; i < idx; ++i)
    {
        ...
        pid_ref = One_Pid_Ctrl(pid_ref,measure->speed_aps,&Chassis_speed_pid);
//        float temp = pid_ref;
        
        // 【保护机制】 输出限幅 (Output Saturation)
        if (pid_ref > SAFETY_CURRENT_LIMIT)
            pid_ref = SAFETY_CURRENT_LIMIT;
        else if (pid_ref < -SAFETY_CURRENT_LIMIT) 
            pid_ref = -SAFETY_CURRENT_LIMIT;
        
        pid_ref = CurrentRamp(pid_ref, last_current_ref[i], 500.0f);
        last_current_ref[i] = pid_ref;
        // 获取最终输出
        set = (int16_t)pid_ref;
...
//	
//	static int16_t lf_out,rf_out,rb_out,lb_out;
//	
//	
//	lf_out =(int16_t)One_Pid_Ctrl(rc_ctrl.vt_lf,dji_motor_instance[0]->measure.speed_aps,&Chassis_speed_pid_lf);
//	rf_out =(int16_t)One_Pid_Ctrl(rc_ctrl.vt_rf,dji_motor_instance[1]->measure.speed_aps,&Chassis_speed_pid_lf);
//	rb_out =(int16_t)One_Pid_Ctrl(rc_ctrl.vt_rb,dji_motor_instance[2]->measure.speed_aps,&Chassis_speed_pid_lf);
//	lb_out =(int16_t)One_Pid_Ctrl(rc_ctrl.vt_lb,dji_motor_instance[3]->measure.speed_aps,&Chassis_speed_pid_lf);
//	
//	
//	sender_assignment[1].tx_buff[0] = (uint8_t)(lf_out >> 8);         // 低八位
//    sender_assignment[1].tx_buff[1] = (uint8_t)(lf_out & 0x00ff); // 高八位
//	
//	sender_assignment[1].tx_buff[2] = (uint8_t)(rf_out >> 8);         // 低八位
//    sender_assignment[1].tx_buff[3] = (uint8_t)(rf_out & 0x00ff); // 高八位
//	
//	sender_assignment[1].tx_buff[4] = (uint8_t)(rb_out >> 8);         // 低八位
//    sender_assignment[1].tx_buff[5] = (uint8_t)(rb_out & 0x00ff); // 高八位

//	sender_assignment[1].tx_buff[6] = (uint8_t)(lb_out >> 8);         // 低八位
//    sender_assignment[1].tx_buff[7] = (uint8_t)(lb_out & 0x00ff); // 高八位




//   if(dji_motor_instance[1]->stop_flag == MOTOR_STOP)
//	{
//      memset(sender_assignment[1].tx_buff, 0, sizeof(sender_assignment[1].tx_buff));
//    }
//		



//    CANTransmit(&sender_assignment[1], 1);
