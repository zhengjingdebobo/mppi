...
static uint32_t can_hcan1_fifo0_callback_count = 0u;
static uint32_t can_hcan1_fifo1_dispatch_count = 0u;
static uint32_t can_hcan1_rx_msg_count = 0u;
static uint32_t can_last_ext_id = 0u;
static uint8_t can_last_ide = 0u;
static uint8_t can_last_dlc = 0u;

static void CANFIFOxCallback(CAN_HandleTypeDef *_hcan, uint32_t fifox);

void CANDispatchRxFifo1(CAN_HandleTypeDef *hcan)
{
    if (hcan == &hcan1)
    {
        can_hcan1_fifo1_dispatch_count++;
    }
...
uint32_t CANGetHcan1Fifo0CallbackCount(void)
{
    return can_hcan1_fifo0_callback_count;
}

uint32_t CANGetHcan1Fifo1DispatchCount(void)
{
    return can_hcan1_fifo1_dispatch_count;
}

uint32_t CANGetHcan1RxMsgCount(void)
{
    return can_hcan1_rx_msg_count;
}

uint32_t CANGetLastExtId(void)
{
    return can_last_ext_id;
}

uint8_t CANGetLastIde(void)
{
    return can_last_ide;
}

uint8_t CANGetLastDlc(void)
{
    return can_last_dlc;
}
...
static void CANAddFilter(CANInstance *_instance)
{
    ...
    can_filter_conf.FilterFIFOAssignment = CAN_RX_FIFO0;  	//修改所有的报文都会在fifo 0// 奇数id的模块会被分配到FIFO0,偶数id的模块会被分配到FIFO1
...
static void CANServiceInit()
{
    ...
    HAL_CAN_ActivateNotification(&hcan2, CAN_IT_RX_FIFO0_MSG_PENDING | CAN_IT_RX_FIFO1_MSG_PENDING | CAN_IT_ERROR | CAN_IT_BUSOFF | CAN_IT_LAST_ERROR_CODE);
...
static void CANFIFOxCallback(CAN_HandleTypeDef *_hcan, uint32_t fifox)
{
    ...
    while (HAL_CAN_GetRxFifoFillLevel(_hcan, fifox)) // FIFO不为空,有可能在其他中断时有多帧数据进入
    {
        ...
        if (_hcan == &hcan1)
        {
            if (fifox == CAN_RX_FIFO0)
            {
                can_hcan1_fifo0_callback_count++;
            }
            can_hcan1_rx_msg_count++;
            can_last_ide = (uint8_t)rxconf.IDE;
            can_last_dlc = rxconf.DLC;
            if (rxconf.IDE == CAN_ID_EXT)
            {
                can_last_ext_id = rxconf.ExtId;
            }
        }
        HWT9053CAN_RecordRaw(_hcan, &rxconf, can_rx_buff);
        VESCMotorProcessCANRx(_hcan, &rxconf, can_rx_buff);
        for (size_t i = 0; i < idx; ++i)
        { // 两者相等说明这是要找的实例
