...
#   4. 单独绘制“PC 指定速度 - STM32 cmd_output”，用于观察串口遥测延迟；
#   5. 默认将 PNG 保存到 CSV 所在目录。
#
...
def plot_velocity_tracking(
    ...
    elapsed, target, stm32_cmd, actual = load_velocity_csv(csv_path)
    ...
    fig, axes = plt.subplots(
        3 if has_stm32_cmd else 2,
        1,
        figsize=(11, 8 if has_stm32_cmd else 7),
        sharex=True,
        gridspec_kw={"height_ratios": (3, 1, 1) if has_stm32_cmd else (3, 1)},
    ...
    if has_stm32_cmd:
        axes[2].plot(
            ...
            color="tab:purple",
            linewidth=1.2,
            label="PC target - delayed STM32 cmd_output",
        )
        axes[2].axhline(0.0, color="black", linewidth=0.8, alpha=0.6)
        axes[2].set_ylabel("Timeline gap\n(m/s)")
