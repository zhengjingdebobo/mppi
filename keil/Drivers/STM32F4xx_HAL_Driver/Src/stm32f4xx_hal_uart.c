...
HAL_StatusTypeDef HAL_UART_Transmit_DMA(UART_HandleTypeDef *huart, const uint8_t *pData, uint16_t Size)
{
  ...
  if (huart->gState == HAL_UART_STATE_READY)
  {
    ...
    huart->pTxBuffPtr = pData; // 发送指令帧缓缓冲区首地址
    huart->TxXferSize = Size; // 总发送长度
    huart->TxXferCount = Size;// 记录剩余发送长度（初始等于总长度，DMA传输中递减）

    huart->ErrorCode = HAL_UART_ERROR_NONE;
    huart->gState = HAL_UART_STATE_BUSY_TX;// 串口全局状态为发送中

    // DMA中断回调函数
    // DMA 完成不同阶段的传输后，会触发对应的回调函数，通知 CPU 处理
...
HAL_StatusTypeDef HAL_UART_Receive_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
  ...
  if (huart->RxState == HAL_UART_STATE_READY)
  {
    ...
    /* Set Reception type to Standard reception */
    huart->ReceptionType = HAL_UART_RECEPTION_STANDARD; // 切换串口 DMA 接收模式
...
HAL_StatusTypeDef HAL_UARTEx_ReceiveToIdle_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
  ...
  if (huart->RxState == HAL_UART_STATE_READY)
  {
    ...
    /* Set Reception type to reception till IDLE Event*/ // 接收的判断条件
    huart->ReceptionType = HAL_UART_RECEPTION_TOIDLE;
    huart->RxEventType = HAL_UART_RXEVENT_TC;
    ...
    // 空闲中断
      ...
      {
        __HAL_UART_CLEAR_IDLEFLAG(huart);
        ATOMIC_SET_BIT(huart->Instance->CR1, USART_CR1_IDLEIE); // 使能空闲中断开启检测
      }
      else
      {
        /* In case of errors already pending when reception is started,
